...
@section('page_subtitle', 'Phòng: '.$room->name)
...
<div class="detail-card">
    <div class="mb-4">
        <div class="text-sm text-slate-400 mb-1">ID: {{ $room->id }}</div>
        <div class="text-xl font-semibold">{{ $room->name }}</div>
        <div class="text-xs text-slate-400 mt-1">
            Rạp: {{ optional($room->cinema)->name ?? '—' }}
        </div>
    </div>
    ...
    <div class="space-y-2">
        ...
        <div class="detail-row">
            ...
            <span class="detail-value">
                {{ optional($room->cinema)->city ?? '—' }}
            </span>
        </div>
        <div class="detail-row">
            <span class="detail-label">Tổng số ghế</span>
            <span class="detail-value">
                {{ $room->total_seats }}
            </span>
        </div>
        <div class="detail-row">
            ...
            <span class="detail-value">
                {{ optional($room->created_at)->format('d/m/Y H:i') ?? '—' }}
            </span>
        </div>
    </div>

    <div class="mt-6 flex justify-between">
        <a href="{{ route('admin.rooms.index') }}" class="text-xs text-slate-400 hover:text-slate-200">
            ← Quay lại danh sách
        </a>
