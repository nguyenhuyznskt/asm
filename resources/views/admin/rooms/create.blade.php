...
@section('page_title', 'Thêm phòng chiếu')
@section('page_subtitle', 'Tạo mới phòng chiếu cho rạp')
    ...
    <form action="{{ route('admin.rooms.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="cinema_id" class="room-select">
                ...
                @foreach($cinemas as $c)
                    <option value="{{ $c->id }}" @selected(old('cinema_id') == $c->id)>
                        {{ $c->name }} ({{ $c->city }})
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="name"
                   value="{{ old('name') }}" class="room-input"
        ...
        <div>
            ...
            <input id="room-total-seats" type="number" name="total_seats"
                   value="{{ old('total_seats', 60) }}" class="room-input" min="0">
