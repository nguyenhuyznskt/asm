...
@section('page_title', 'Sửa suất chiếu')
@section('page_subtitle', 'Chỉnh sửa suất chiếu ID: '.$showtime->id)
    ...
    async function loadSlots() {
        ...
        try {
            ...
            slots.forEach(hhmm => {
                ...
                if (hhmm === currentStart) {
                    opt.selected = true;
                }
                timeSelect.appendChild(opt);
            });

            if (!slots.includes(currentStart)) {
    ...
    <form action="{{ route('admin.showtimes.update', $showtime) }}" method="POST" class="space-y-4">
        ...
        <div class="grid md:grid-cols-2 gap-3">
                ...
                <select id="st-movie" name="movie_id" class="st-select">
                    @foreach($movies as $movie)
                        <option value="{{ $movie->id }}"
                                @selected(old('movie_id', $showtime->movie_id) == $movie->id)>
                            {{ $movie->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                @php
                    $currentCinemaId = optional($showtime->room->cinema ?? null)->id;
                @endphp
                <select id="st-cinema" class="st-select">
                    <option value="">-- Tất cả rạp --</option>
                    @foreach($cinemas as $cinema)
                        <option value="{{ $cinema->id }}"
                                @selected($currentCinemaId == $cinema->id)>
                            {{ $cinema->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid md:grid-cols-2 gap-3">
            <div>
                ...
                @php
                    ...
                    $days     = 7;
                    $selected = old('show_date', $show_date);
                @endphp
